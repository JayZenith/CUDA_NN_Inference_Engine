...
__global__ void vectorSquared(const float* A, const float* B, float* C, int N){
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
int main() {
    ...
    cudaDeviceSynchronize(); // Wait for vector Add to finish

    vectorSquared<<<blocks, threadsPerBlock>>>(d_A, d_B, d_C, N);
    cudaDeviceSynchronize(); // wait for vector Squared to finish
